...
class StoreRepository implements StoreRepositoryInterface {
  ...
  Future<ItemModel?> getStoreItemList(
    ...
    Response response = await apiClient.getData(
        '${AppConstants.categoryItemUri}${categoryID ?? 0}?store_id=$storeID&offset=$offset&limit=13&type=$type&filter=$filterString&rating_count=${rating ?? ''}&min_price=${lowerValue ?? ''}&max_price=${upperValue ?? ''}');
    if (response.statusCode == 200) {
      storeItemModel = ItemModel.fromJson(response.body);
      if (storeID != null && storeItemModel.items != null) {
        storeItemModel.items!.removeWhere((item) => item.storeId != storeID);
      }
    }
