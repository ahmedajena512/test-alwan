...
class CartController extends GetxController implements GetxService {
  ...
  double calculationCart() {
    ...
    for (var cartModel in cartList) {
      bool isFood = ModuleHelper.getModuleConfig(cartModel.item!.moduleType)
          .newVariation!;
      ...
      double currentItemVariationPrice =
          cartServiceInterface.calculateVariationPrice(
              isFood, cartModel, discount, discountType, 0);
      double currentItemVariationWithoutDiscountPrice = cartServiceInterface
          .calculateVariationWithoutDiscountPrice(isFood, cartModel, 0);
      bool haveVariation =
          cartServiceInterface.checkVariation(isFood, cartModel);

      if (isFood) {
        double itemBasePrice = cartModel.item!.price! * cartModel.quantity!;
        double itemBasePriceWithDiscount = PriceConverter.convertWithDiscount(
                cartModel.item!.price!, discount, discountType,
                isFoodVariation: true)! *
            cartModel.quantity!;
        _itemPrice += itemBasePrice;
        _itemDiscountPrice += (itemBasePrice - itemBasePriceWithDiscount) +
            (currentItemVariationWithoutDiscountPrice -
                currentItemVariationPrice);
        _variationPrice += currentItemVariationWithoutDiscountPrice;
      } else {
        if (haveVariation) {
          _itemPrice += currentItemVariationWithoutDiscountPrice;
          _itemDiscountPrice += (currentItemVariationWithoutDiscountPrice -
              currentItemVariationPrice);
          _variationPrice += currentItemVariationWithoutDiscountPrice;
        } else {
          _itemPrice += (cartModel.item!.price! * cartModel.quantity!);
          _itemDiscountPrice += (cartModel.item!.price! * cartModel.quantity!) -
    ...
    _subTotal = (_itemPrice - _itemDiscountPrice) +
        _addOns +
        (ModuleHelper.getModule()?.moduleType == 'food' ? _variationPrice : 0);
