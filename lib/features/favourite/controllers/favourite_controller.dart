...
class FavouriteController extends GetxController implements GetxService {
  ...
  bool get isLoading => _isRemoving; // Alias for consistency with UI
  ...
  Future<void> clearAll() async {
    ...
    // Clear all items
    List<int?> itemIdsCopy = List.from(_wishItemIdList);
    for (int? id in itemIdsCopy) {
      if (id != null) {
        await favouriteServiceInterface.removeFavouriteList(id, false);
      }
    }

    // Clear all stores
    List<int?> storeIdsCopy = List.from(_wishStoreIdList);
    for (int? id in storeIdsCopy) {
      if (id != null) {
        await favouriteServiceInterface.removeFavouriteList(id, true);
      }
    }
    ...
    update();
    Get.back();
    showCustomSnackBar('all_favourites_cleared'.tr, isError: false);
  }
}
